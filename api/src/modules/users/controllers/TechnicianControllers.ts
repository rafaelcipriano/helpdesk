...
const userUseCase = new UserUseCase(prisma.technician)

class TechnicianControllers {
  async create(req: Request, res: Response) {
    const data = req.body

    const technician = await userUseCase.create(data)
    
    return res.status(201).json(technician)
  }

  async index(req: Request, res: Response) {
    const technicians = await userUseCase.index()
